...
typedef struct TpTcpStats_ {
    ...
    uint64_t data_toserver, data_toclient;
...
static void emit_tcp_signal(SCJsonBuilder *jb, const TpTcpStats *s)
{
    ...
    /* Raw payload byte totals per direction. The downstream pipeline
     * derives PCR (producer/consumer ratio) from these. */
    SCJbSetUint(jb, "data_toserver", s->data_toserver);
    SCJbSetUint(jb, "data_toclient", s->data_toclient);
