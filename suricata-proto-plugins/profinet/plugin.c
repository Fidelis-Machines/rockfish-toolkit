/* Fidelis Farm & Technologies, LLC PROFINET DCP Parser Plugin for Suricata
 * Copyright 2025-2026. Fidelis Farm & Technologies, LLC
 * SPDX-License-Identifier: GPL-2.0-only
 */

#include "suricata-plugin.h"
#include "suricata-common.h"
#include "util-debug.h"

#include "profinet-plugin.h"

extern void ProfinetParserRegister(void);

static void SCPluginInit(void)
{
    SCLogNotice("Loading Rockfish PROFINET DCP parser plugin");

    SCAppLayerPlugin plugin = {
        .name = "profinet",
        .Register = ProfinetParserRegister,
        .KeywordsRegister = NULL,
        .logname = "profinet",
        .confname = "profinet",
        .dir = 0,
        .Logger = NULL,
    };
    SCPluginRegisterAppLayer(&plugin);
}

const SCPlugin PluginRegistration = {
    .version = SC_API_VERSION,
    .suricata_version = SC_PACKAGE_VERSION,
    .name = "rockfish-profinet-parser",
    .plugin_version = PROFINET_PLUGIN_VERSION,
    .author = "Fidelis Farm & Technologies, LLC",
    .license = "GPL-2.0-only",
    .Init = SCPluginInit,
};

const SCPlugin *SCPluginRegister(void)
{
    return &PluginRegistration;
}
