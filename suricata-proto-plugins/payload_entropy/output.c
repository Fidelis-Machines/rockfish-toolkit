 ...
 * `emit:` block in suricata.yaml. PCR (producer/consumer ratio) used to
 * be emitted here over the sampled byte window; it now lives downstream
 * of the transport_signals plugin, computed from full-flow
 * `data_toserver` / `data_toclient` totals.
 */
...
void RockfishPayloadEntropyRegister(void)
{
    ...
    SCLogNotice("    Emit:                      entropy=%s splt=%s",
                emit_entropy ? "yes" : "no",
                emit_splt    ? "yes" : "no");
