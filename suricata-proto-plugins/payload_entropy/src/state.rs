...
//! Per-flow tracker for two signals (each emit-toggleable):
...
impl State {
    ...
    pub fn observe(
        ...
        // Histogram + bytes_sampled (gated on entropy emit). When entropy
        // is off we still bump packets_sampled so the per-direction cap
        // drives SPLT length too.
        let dir = if direction == 0 { &mut flow.to_server } else { &mut flow.to_client };
        if track_hist {
