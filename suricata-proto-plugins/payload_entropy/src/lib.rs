...
//!
//! Two signals per flow, each toggleable:
//!   1. Shannon byte entropy (bits/byte) per direction
//!   2. SPLT (Sequence of Packet Lengths and Times) — three index-aligned
//!      views: `splt_lengths` (u16), `splt_iats_us` (u32), `splt` (letter
//!      string with case = direction).
//!
//! PCR (producer/consumer ratio) was previously emitted here over the
//! sampled byte window; it now lives downstream of the transport_signals
//! plugin, computed from full-flow `data_toserver` / `data_toclient`
//! totals instead of a capped sample.
